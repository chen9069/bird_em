    ...
    def hand_summary(self, board, hands):
        ...
        for i in range(len(stages)):
            ...
            for player, hand in enumerate(hands):
                ...
                rank_announced = "Player %d hand = %s, percentage rank among all hands = %f" % (
                    player + 1, class_string, percentage)
                print(rank_announced)
            ...
            if i != stages.index("RIVER"):
                if len(winners) == 1:
                    s = "Player %d hand is currently winning.\n" % (winners[0] + 1,)
                    print(s)
                else:
                    s = "Players %s are tied for the lead.\n" % [x + 1 for x in winners]
                    print(s)

            # otherwise on all other streets
            else:
                print()
                hand_over = ("=" * line_length) + " HAND OVER " + ("=" * line_length)
                print(hand_over)
                
                if len(winners) == 1:
                    win_announced = "Player %d is the winner with a %s\n" % (winners[0] + 1, 
                        self.class_to_string(self.get_rank_class(self.evaluate(hands[winners[0]], board))))
                    print(win_announced)
                else:
                    win_announced = "Players %s tied for the win with a %s\n" % (winners, 
                        self.class_to_string(self.get_rank_class(self.evaluate(hands[winners[0]], board))))
                    print(win_announced)
